...
public class Room5Client extends JFrame implements ActionListener, Runnable {
	...
	public Room5Client(LoginDTO loginDTO, Clip clip, int roomNum) {
		super("Waiting Room");
		roomDAO = new RoomDAO();
	...
	public void service() {
		...
		try {
			// 소켓생성
			socket = new Socket(serverIP, 5400); // 아이피는 위에있는거고 port 는 고정? 꼭 9500써야하나
...
//---------------------------로딩 프레임------------------
class Loading5 extends JFrame implements Runnable {
	private JProgressBar jpb = new JProgressBar(JProgressBar.HORIZONTAL, 0, 100);
	private LoginDTO loginDTO;

	public Loading5(LoginDTO loginDTO) {
	...
	public void run() {
		...
		dispose();
		new Game5Client(loginDTO,5).service();
	}
}

class Panel3_15 extends JPanel {
	public Panel3_15() {
