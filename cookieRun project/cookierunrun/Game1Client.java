...
class Game1Client extends JFrame implements KeyListener, Runnable {
	...
	public BackgroundT1 backgroundT;
	public ImageIcon backgroundImg;
	public RunningCookie1 runningCookie;
	public MovingHurdle1 movingHurdle;
	public static HpBar1 hpBar;
	public GomJellyDummy1 gomJellyDummy;
	public Jelly1 jelly;
	public JellyDrop1 jellyDrop;
	public BigGom1 bigGom;
	public Game1Client myFrame;
	...
	private Game1Client gameClient;
	private Thread t;
	
	private int roomNum;

	// -------------------------메소드--------------------------------
	public Game1Client(LoginDTO loginDTO, int roomNum) {
		this.loginDTO = loginDTO;
		this.roomNum =roomNum;
		nickName=loginDTO.getNickName();
		
		jellyDrop = new JellyDrop1();
		// 곰젤리 클래스 호출
		gomJellyDummy = new GomJellyDummy1();
	...
	public void service() {
		...
		try {
			// 소켓생성
			socket = new Socket(serverIP, 8000); // 아이피는 위에있는거고 port 는 고정? 꼭 9500써야하나
	...
	public void run() {
		...
		while(!t.isInterrupted()) {  
			//게임 끝났을 때
			if(!gameDie) {
				writer.println(Jelly1.gameScore);
				writer.flush();
			} else {
				if (gameDie && temp1 == false) {
					writer.println("gameDie:"+Jelly1.gameScore);
					writer.flush();
					temp1 = true;
					gameEnd();
				}
	...
	// 게임이 종료되면 나오는 frame창
	public void gameEnd() {
		new EndLoading1(loginDTO, this, roomNum);
	}
}

class EndLoading1 extends JFrame implements Runnable {
	private String nickName;
	private Game1Client game;
	private EndGame1 endGame;
	private LoginDTO loginDTO;
	private int roomNum;

	public EndLoading1(LoginDTO loginDTO, Game1Client game, int roomNum) {
		super("로딩 화면");
	...
	public void run() {
		while (true) {
			System.out.println(Game1Client.rivalGameDie);  //지우시면 게임 실행이 안됩니다ㅜㅜㅜ 지우지 마세요!!!
			if (Game1Client.rivalGameDie) {
				game.dispose();
				break;
			}
		}
	...
	public void gameEnd() {
		endGame = new EndGame1(loginDTO, roomNum);
...
class BackgroundT1 extends JPanel implements Runnable {
    // ------------------------필드----------------------
    public int x1 = 0;
    public int x2 = 700;
    // 1라운드 배경 이미지
...
class RunningCookie1 extends JPanel implements Runnable{
	//-------------------필드----------------------
...
class MovingHurdle1 extends JPanel implements Runnable {
    // 1라운드 장애물 이미지 필드
        ...
        if (movingHurdle.size() > 0) {
            for (HurdleDTO data : movingHurdle) {
                if (data.getImageIndex() == 3 || data.getImageIndex() == 4) {
                    g.drawImage((BackgroundT1.round2 ? round2[data.getImageIndex()] : round1[data.getImageIndex()]),
...
class JellyDrop1 implements Runnable {
    public JellyDrop1() {
        Jelly1.list = new ArrayList<JellyDTO>();
    }
    ...
    public void run() {
        while (!Game1Client.gameDie) {
            // 새로운 젤리 셋팅
            if (Jelly1.list.size() == 16)
                Jelly1.list.remove(0);
            JellyDTO jellyDTO = new JellyDTO();
            jellyDTO.setX(720);
            jellyDTO.setY(330);
            Jelly1.list.add(jellyDTO);
...
class Jelly1 extends JPanel implements Runnable {
    // --------------------------필드------------------------
	//이미지 생성
	public Image jelly;
    public Image coin;
...
class GomJellyDummy1 extends JPanel implements Runnable {
    // --------------------------필드-----------------------------------
    // 패널에 넣을 이미지 호출
...
class BigGom1 extends JPanel implements Runnable {
    // ----------------------필드------------------------
    // 이미지 생성 필드
                ...
                if (bigList.get(i).getX() >= 80 && bigList.get(i).getX() <= 180) {
                    if (bigList.get(i).getY() >= cookieY && bigList.get(i).getY() <= cookieY + 100) {
                        if (!bigList.get(i).eat) { // 먹지않은 젤리일 때
                            ...
                            if (bigList.get(i).getRandomIndex() == 1) GomJellyDummy1.countGom += 5;  //곰젤리를 먹었을 경우 곰젤리 점수를 카운트
                            else Jelly1.coinEat += 5;  //코인을 먹었을 경우 코인 수를 카운트
                            Jelly1.gameScore += 5000;
...
class HpBar1 extends JPanel implements Runnable {
	//---------------------필드-----------------------
	// 체력 설정 필드
	public Image HpBar;
	public Image HbBar;
...
//게임종료 클래스 호출
	...
	public void run() {
		while (!Game1Client.gameDie) {
			MovingHurdle1.health -= 4; // 2초에 1퍼센트 씩 체력 소모
			try {
...
class EndGame1 extends JFrame implements ActionListener{
	...
	//패널 선언
	private  JPanel p;
	private  EndBack1 end;
	private int roomNum;
	
	public EndGame1(LoginDTO loginDTO, int roomNum) {
		this.loginDTO = loginDTO;
		this.roomNum = roomNum;
		this.setSize(700, 460);
		this.setLayout(null);
		...
		gameScoreL.setText(df.format(Jelly1.gameScore));
		...
		gomScore.setText(df.format(GomJellyDummy1.countGom));
		//코인 라벨 설정
		...
		if(Game1Client.rivalScore>Jelly1.gameScore) {
			...
			coinScore.setText(df.format(Jelly1.coinEat));
		}
		...
		//코인&update 실행
		loginDTO.setCoin(loginDTO.getCoin()+Jelly1.coinEat);
		loginDTO.setScore((loginDTO.getScore()<Jelly1.gameScore)?Jelly1.gameScore:loginDTO.getScore());
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==yesBtn) {
			...
			BackgroundT1.round2=false;			
			Jelly1.gameScore=0;
			Jelly1.coinEat=0;
			Jelly1.gameScore=0;
			Game1Client.gameDie=false;
			MovingHurdle1.health=100;
