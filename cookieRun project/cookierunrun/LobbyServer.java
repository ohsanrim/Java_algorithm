...
import java.io.IOException; // ServerSocket이거를쓰면 나온다.
...
// 1 ㄷ N 채팅
class LobbyServer { // 회사의 본점 같은 곳
	private ServerSocket serverSocket; // 서버 소켓을 열어 주는거다?
	private List<LobbyHandler> list; // 인터페이스이다 여러개의 소켓 확인용?

	public LobbyServer() {
		try {
			serverSocket = new ServerSocket(1000); // 같은 방안의 포트 번호를 준다.
