...
class Game2Client extends JFrame implements KeyListener, Runnable {
    ...
    public BackgroundT2 backgroundT;
    public ImageIcon backgroundImg;
    public RunningCookie2 runningCookie;
    public MovingHurdle2 movingHurdle;
    public static HpBar2 hpBar;
    public GomJellyDummy2 gomJellyDummy;
    public Jelly2 jelly;
    public JellyDrop2 jellyDrop;
    public BigGom2 bigGom;
    public Game2Client myFrame;
    ...
    private Game2Client gameClient;
    private Thread t;
    
    private int roomNum;

    // -------------------------메소드--------------------------------
    public Game2Client(LoginDTO loginDTO, int roomNum) {
        this.loginDTO = loginDTO;
        this.roomNum =roomNum;
        nickName=loginDTO.getNickName();
        
        jellyDrop = new JellyDrop2();
        // 곰젤리 클래스 호출
        gomJellyDummy = new GomJellyDummy2();
    ...
    public void service() {
        ...
        try {
            // 소켓생성
            socket = new Socket(serverIP, 8100); // 아이피는 위에있는거고 port 는 고정? 꼭 9500써야하나
    ...
    public void run() {
        ...
        while(!t.isInterrupted()) {  
            //게임 끝났을 때
            if(!gameDie) {
                writer.println(Jelly2.gameScore);
                writer.flush();
            } else {
                if (gameDie && temp1 == false) {
                    writer.println("gameDie:"+Jelly2.gameScore);
                    writer.flush();
                    temp1 = true;
                    gameEnd();
                }
    ...
    // 게임이 종료되면 나오는 frame창
    public void gameEnd() {
        new EndLoading2(loginDTO, this, roomNum);
    }
}

class EndLoading2 extends JFrame implements Runnable {
    private String nickName;
    private Game2Client game;
    private EndGame2 endGame;
    private LoginDTO loginDTO;
    private int roomNum;

    public EndLoading2(LoginDTO loginDTO, Game2Client game, int roomNum) {
        super("로딩 화면");
    ...
    public void run() {
        while (true) {
            System.out.println(Game2Client.rivalGameDie);  //지우시면 게임 실행이 안됩니다ㅜㅜㅜ 지우지 마세요!!!
            if (Game2Client.rivalGameDie) {
                game.dispose();
                break;
            }
        }
    ...
    public void gameEnd() {
        endGame = new EndGame2(loginDTO, roomNum);
...
class BackgroundT2 extends JPanel implements Runnable {
    // ------------------------필드----------------------
    public int x1 = 0;
    public int x2 = 700;
    // 1라운드 배경 이미지
...
class RunningCookie2 extends JPanel implements Runnable{
    //-------------------필드----------------------
    ...
    public void run() {
        ...
        while(!Game2Client.gameDie) {
            ...
            repaint();
            if(Game2Client.gameDie==true) break;
        }
    }
...
class MovingHurdle2 extends JPanel implements Runnable {
    // 1라운드 장애물 이미지 필드
        ...
        if (movingHurdle.size() > 0) {
            for (HurdleDTO data : movingHurdle) {
                if (data.getImageIndex() == 3 || data.getImageIndex() == 4) {
                    g.drawImage((BackgroundT2.round2 ? round2[data.getImageIndex()] : round1[data.getImageIndex()]),
...
class JellyDrop2 implements Runnable {
    public JellyDrop2() {
        Jelly2.list = new ArrayList<JellyDTO>();
    }
    ...
    public void run() {
        while (!Game2Client.gameDie) {
            // 새로운 젤리 셋팅
            if (Jelly2.list.size() == 16)
                Jelly2.list.remove(0);
            JellyDTO jellyDTO = new JellyDTO();
            jellyDTO.setX(720);
            jellyDTO.setY(330);
            Jelly2.list.add(jellyDTO);
...
class Jelly2 extends JPanel implements Runnable {
    // --------------------------필드------------------------
    //이미지 생성
    public Image jelly;
    public Image coin;
...
class GomJellyDummy2 extends JPanel implements Runnable {
    // --------------------------필드-----------------------------------
    // 패널에 넣을 이미지 호출
    ...
    public void run() {
        while (!Game2Client.gameDie) {
            ...
            if (Game2Client.gameDie == true) break;  //확인사살로 한개 더 만들어 둠
        }
    }
    //음악을 재생해주는 메소드
    public void play(String fileName) {
...
class BigGom2 extends JPanel implements Runnable {
    // ----------------------필드------------------------
    // 이미지 생성 필드
                ...
                if (bigList.get(i).getX() >= 80 && bigList.get(i).getX() <= 180) {
                    if (bigList.get(i).getY() >= cookieY && bigList.get(i).getY() <= cookieY + 100) {
                        if (!bigList.get(i).eat) { // 먹지않은 젤리일 때
                            ...
                            if (bigList.get(i).getRandomIndex() == 1) GomJellyDummy2.countGom += 5;  //곰젤리를 먹었을 경우 곰젤리 점수를 카운트
                            else Jelly2.coinEat += 5;  //코인을 먹었을 경우 코인 수를 카운트
                            Jelly2.gameScore += 5000;
    ...
    @Override
    public void run() {
        while (!Game2Client.gameDie) {   //게임 종료시까지 플레이
            endTime = System.currentTimeMillis();
            gamingTime = (int) (endTime - startTime) / 1000;
            for (BigDTO data : bigList) {
...
class HpBar2 extends JPanel implements Runnable {
    //---------------------필드-----------------------
    // 체력 설정 필드
    public Image HpBar;
    public Image HbBar;
...
//게임종료 클래스 호출
    ...
    public void paintComponent(Graphics g) {
        ...
        g.drawImage(HbBar, 130, 38, (MovingHurdle2.health * 4), 20, this);
        g.drawImage(HpBar, 90, 20, 450, 50, this);
        g.drawImage(rivalScoreImg, 10,5,80,40,this);
        
        //죽었을 때(체력이 0보다 작을 때)
        if (MovingHurdle2.health < 0) {
            Game2Client.gameDie = true;
        }
        // 상단 위에 곰젤리&코인을 카운팅하는 위젯 넣기
        g.drawImage(gomCountBackground, 570, 30, 35, 30, this);
        gomCount.setText(Integer.toString(GomJellyDummy2.countGom));
        
        g.drawImage(coinCountBackground, 570, 75, 35, 30, this);
        coinCount.setText(Integer.toString(Jelly2.coinEat));
        
        g.drawImage(scoreBackground, 300, 75, 20, 20, this);
        score.setText(df.format(Jelly2.gameScore));
    
        competitionL.setText(Game2Client.competitionScore);
    }
    //스레드를 시작해주는 메소드
    public void threadStart() {
        Thread t = new Thread(this);
    ...
    public void run() {
        while (!Game2Client.gameDie) {
            MovingHurdle2.health -= 4; // 2초에 1퍼센트 씩 체력 소모
            try {
            ...
            repaint();
            if (Game2Client.gameDie) break;
        }
    }
}
class EndGame2 extends JFrame implements ActionListener{
    ...
    //패널 선언
    private  JPanel p;
    private  EndBack2 end;
    private int roomNum;
    
    public EndGame2(LoginDTO loginDTO, int roomNum) {
        this.loginDTO = loginDTO;
        this.roomNum = roomNum;
        this.setSize(700, 460);
        this.setLayout(null);
        ...
        JPanel bubbleP = new JPanel() {
            ...
            public void paintComponent(Graphics g) {
                ...
                if(Game2Client.rivalScore>Jelly2.gameScore) {
        ...
        gameScoreL.setText(df.format(Jelly2.gameScore));
        ...
        gomScore.setText(df.format(GomJellyDummy2.countGom));
        //코인 라벨 설정
        ...
        if(Game2Client.rivalScore>Jelly2.gameScore) {
            ...
            coinScore.setText(df.format(Jelly2.coinEat));
        }
        ...
        //코인&update 실행
        loginDTO.setCoin(loginDTO.getCoin()+Jelly2.coinEat);
        loginDTO.setScore((loginDTO.getScore()<Jelly2.gameScore)?Jelly2.gameScore:loginDTO.getScore());
    ...
    public void actionPerformed(ActionEvent e) {
        if(e.getSource()==yesBtn) {
            ...
            BackgroundT2.round2=false;          
            Jelly2.gameScore=0;
            Jelly2.coinEat=0;
            Jelly2.gameScore=0;
            Game2Client.gameDie=false;
            MovingHurdle2.health=100;
            Game2Client.rivalGameDie=false;
            Game2Client.competitionScore="0";
