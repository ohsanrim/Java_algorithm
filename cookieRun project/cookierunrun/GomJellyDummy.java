...
public class GomJellyDummy extends JPanel implements Runnable {
	//--------------------------필드-----------------------------------
	//패널에 넣을 이미지 호출
	...
	public void paintComponent(Graphics g) {
		...
		//곰젤리 이미지 생성
		gom  = t.getImage("src/Jelly/Gom_jelly2.png");
		jellyDisappear =t.getImage("src/Jelly/afterGom.gif");
		...
		if(dummy.size()>0) {
			for(GomJellyDTO data2 : dummy) {
				g.drawImage(data2.eat?jellyDisappear:gom,data2.getX(),data2.getY(),35,30,this);  //먹힌 젤리면 jellyDisappear , 안먹혔으면 gom
			}
		}
		//프레임 밖을 벗어나면 리스트 지워버리기
		if(dummy.size()>0) {
			if(dummy.get(39).getX()<-20) dummy.clear();   //잴리 개수 변경 시 이 값도 변경 필요
		}
	}
	//스레드 스타트 메소드
